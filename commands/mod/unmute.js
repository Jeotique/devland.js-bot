...
module.exports = {
    data: new Discord.GuildCommand({
        ...
        options: [{
            ...
            name: "user",
            description: "Who you want to remove the timeout ?",
            required: true
        }, {
    ...
    run: async (client, interaction) => {
        ...
        if(!member.communication_disabled_until) return interaction.followUp({content: `This person is not muted`}).catch(e=>{})
        member.edit({
            communication_disabled_until: null,
            reason: reason
        }).then(() => {
            interaction.followUp({content: `${user.tag} has been unmuted`})
        }).catch(e=>{
            interaction.followUp({content: `${user.tag} cannot be unmuted`})
        })
    }
}